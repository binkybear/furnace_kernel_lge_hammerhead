...
static int tempC_max;
static int control_strength;
static int control_switch;
static DEFINE_MUTEX(vdd_rstr_mutex);
static DEFINE_MUTEX(psm_mutex);

module_param(control_strength, int, 0664);
module_param(control_switch, int, 0664);
...
static int qpnp_get_battery_temp(int btemp)
{
	int ret = 0;
	...
	btemp = (int)results.physical;

	return 0;
}
#endif

static int max17048_get_prop_temp(int batt_temp)
{
#ifdef CONFIG_SENSORS_QPNP_ADC_VOLTAGE
	int ret;

	ret = qpnp_get_battery_temp(batt_temp);
	if (ret)
...
#ifdef CONFIG_SMP
static void __ref do_batt_control()
{
	int battery_temp;
	int ret;

	if (control_strength > 0) {
		ret = max17048_get_prop_temp(battery_temp);
		pr_debug("%s: battery_temperature = %d\n", __func__, ret);
	}
}
#else
static void do_batt_control()
{
	return;
}
#endif
